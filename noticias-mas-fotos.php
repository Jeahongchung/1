...
<body>
  ...
  function list_foto(){
    ...
    while($i<count($list)){
      if($list[$i] != '.'){
        if($list[$i] != '..'){
          if($i < 16 ){ $filename[$i]=substr($list[$i], 0, -4); $result=$list[$i]; $filetitle[$i]=substr($filename[$i],0,25);
          ?>
          <div style="width:200px; height:140px; float:left; margin-top:0px; margin-left:10px;">
            <p style="margin-top:10; font-size:15px; margin-left:10px; color:blue;"><?=$filetitle[$i]?></p>
